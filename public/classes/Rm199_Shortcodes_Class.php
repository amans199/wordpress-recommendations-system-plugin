...
?>
    ...
    public static function rm199_posts($attr)
    {
        ...
        if (!isset($attr['keywords_selection']) || isset($attr['latest_posts']) || !is_user_logged_in()) {            // get the latest posts 
            ...
            if ($query->have_posts()) {
                ob_start();
                Rm199_Shortcodes_Class::rm199_styles($main_color, $secondary_color, $text_color);
    ...
    public static function rm199_input($attr)
    {
        ...
        if (is_user_logged_in()) {
            ...
            <?php $all_preferences_shown = explode(",", trim($current_user_preferences[0]->post_content));
            echo '<div class="rm199__keywords">';
            ...
            for ($p = 0; $p < count($all_preferences_shown); $p++) {
                if (!empty(trim($all_preferences_shown[$p]))) {
                    echo '<span class="rm199__keyword"><span class="rm199__keyword__content">' .  $all_preferences_shown[$p] . '</span> <span onClick="deleteThisKeyword(event)" class="dashicons dashicons-no-alt"></span></span>';
                }
            }
            echo '</div>';
            ?>
...
<?php
            ...
            if (isset($_POST['rm199-submitted'])) {
                ...
                // $headers = "From: Ahmed Mansour <ahmed.ouda1997@gmail.com>" . "\r\n";

                // If email has been process for sending, display a success message

                // count all user posts 
                $users_preferences_posts_args = array(
                    'author'        =>   get_current_user_id(),
                    'post_type' => 'user_preference'
                );
                $users_preferences_posts = count(get_posts($users_preferences_posts_args));

                // Insert the post into the database

                if ($users_preferences_posts < 1) {
                    $new_preferences = array(
                        'post_title'    =>  $name,
                        'post_content'  => wp_strip_all_tags($subject),
                        'post_status'   => 'publish',
                        'post_type' => 'user_preference',
                        'post_author'   => get_current_user_id(),
                    );
                    wp_insert_post($new_preferences);
                } else {
                    $args = array(
                        'author'        =>   get_current_user_id(),
                        'post_type' => 'user_preference',
                    ...
                    $current_user_preferences = get_posts($args);
                    $old_content = $current_user_preferences[0]->post_content;
                    $update_preferences = array(
                        'ID'           => $current_user_preferences[0]->ID,
                        'post_title'    =>  $name,
                        'post_content'  => $old_content . ',' . wp_strip_all_tags($subject),
                        'post_status'   => 'publish',
                        'post_type' => 'user_preference',
                        'post_author'   => get_current_user_id(),
                    );
                    wp_update_post($update_preferences);
                }
            }
            return ob_get_clean();
        }
